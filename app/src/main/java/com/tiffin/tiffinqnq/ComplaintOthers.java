        ...
        btnpost.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(edtdescription.isEmpty()){
                    Toast.makeText(ComplaintOthers.this, "Please add description first", Toast.LENGTH_SHORT).show();
                }else {
                    progressBar.setVisibility(View.VISIBLE);
                    DocumentReference docIdRef = fstore.collection("users").document(userID);
                    docIdRef.collection("complaints").document("other").
                            collection("Other").document().set(data).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    Toast.makeText(ComplaintOthers.this, "Complaints submitted successfully", Toast.LENGTH_SHORT).show();
                                    Intent intent=new Intent(getApplicationContext(),MainActivity.class);
                                    startActivity(intent);
                                    finish();
                                    progressBar.setVisibility(View.INVISIBLE);
                                    //Toast.makeText(ComplaintOthers.this, "done..", Toast.LENGTH_SHORT).show();
                                }
                            });
                }
